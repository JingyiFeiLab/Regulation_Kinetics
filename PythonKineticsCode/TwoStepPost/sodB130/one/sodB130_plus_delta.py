...
def lnprior(args):
    ...
    if not (1E-6 <= k_on <= 1E-2 and 1E-3 <= k_off <= 10.0 and .01 <= kx_s_p <= 1.10 and .01 <= c <= 1.00 and .01 <= d <= 1.00 and 1E-8 <= b_e <= 1E-3 and kx_wt_ml - 1.01*sigma_kx_wt <= kx_wt <= kx_wt_ml + 1.01*sigma_kx_wt and kx_rne_ml - 1.01*sigma_kx_rne <= kx_rne <= kx_rne_ml + 1.01*sigma_kx_rne and b_m <= b_ms <= 0.8):
        return -np.inf
