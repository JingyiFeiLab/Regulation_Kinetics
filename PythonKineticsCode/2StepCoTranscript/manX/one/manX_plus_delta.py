...
def lnprior(args):
    ...
    if not (1E-9 <= k_on <= 1E-2 and 1E-5 <= k_off <= 10.0 and .01 <= kx_s_p <= 1.10 and .01 <= c <= 1.00 and .01 <= d <= 1.00 and 1E-6 <= b_e <= 1.0 and kx_wt_ml - 1.01*sigma_kx_wt <= kx_wt <= kx_wt_ml + 1.01*sigma_kx_wt and kx_rne_ml - 1.01*sigma_kx_rne <= kx_rne <= kx_rne_ml + 1.01*sigma_kx_rne and b_m <= b_ms):
        return -np.inf
...
def fixPos():
    for i in range(len(pos)):
        ...
        pos[i][1] = np.clip(pos[i][1], 1E-5, 10.0) #k_off
        ...
        pos[i][4] = np.clip(pos[i][4], b_m_ml,.05)#b_m_s
