...
WalkerFolder1 = "/Users/reyer/Data/Python_Simulations/8420/post/sodB130/four/"
...
for i in range(posteriorLike):
    if (posteriorShaped[i] < -1000):
        posteriorShaped[i] = -1000
...
while max_count < 5:
    ...
    k_on,k_off,kx_s_p,b_e,b_ms,kx_wt,kx_rne = walkersShaped[:][np.argmax(posteriorShaped)]
    ...
    if k_on == k_on_p and k_off == k_off_p and kx_s_p == kx_s_p_p and b_e == b_e_p and kx_wt == kx_wt_p and kx_rne == kx_rne_p and b_ms == b_ms_p:
        continue
    else:
        k_on_p = k_on
        k_off_p = k_off
...
#        if posterior[j,i] < posterior_check:
#            continue
#        b_ms_data.append(data1_values[i + step_count][13])
#    step_count += num_steps          
...
#p = norm.pdf(x, fit_alpha, fit_loc)
#plt.plot(x, p, 'k', linewidth=2)
#title = "b_ms_rne : mu = %.6f,  std = %.6f , MLE = %.6f" % (norm.mean(fit_alpha, fit_loc), norm.std(fit_alpha, fit_loc), b_ms_rne_max)
#plt.title(title)
#plt.savefig(WalkerFolder1 + "b_ms_rne_fit.png")
#plt.close()
