...
yData[1, 0, :] = [134917.1029, 184452.1771, 180579.3257,216245.1063, 432250.9255, 839879.6454, 1170873.494, 1903248.354]
yData[1, 1, :] = [22443.71086, 339380.1687, 774276.8132, 989754.3786, 1178364.294, 1168843.171, 1039977.762, 1152377.24]
yData[1, 2, :] = [1299487.304, 2158748.53, 1461797.642, 1479650.818, 1213052.738, 1132760.79, 930608.6544, 743058.2862]
yData[1, 3, :] = [257171.8156, 242054.4163, 248115.1836, 246027.831, 383643.0025, 802367.1203, 1131430.97, 1382649.746]
yData[1, 4, :] = [16225.77785, 7646.443305, 194166.3494, 714018.5826, 1418654.816, 1533096.423, 1617435.188, 2046367.339]
yData[1, 5, :] = [2188990.237, 2050903.278, 2390768.473, 1788277.574, 1296136.946, 1018016.032, 973140.0654, 973140.0654]

yData[0, 0, :] = [144202.1426, 333919.142, 149161.7576, 304584.7737, 738063.1426, 808832.9779, 1392651.226, 1681171.266]
yData[0, 1, :] = [33318.96697, 262752.4716, 627298.9573, 851325.6287, 1159651.072, 1151829.933, 1016580.917, 1118273.902]
yData[0, 2, :] = [1788327.436, 1787030.495, 1406293.534, 1188055.269, 1088649.355, 825880.4722, 785990.9093, 744898.0836]
yData[0, 3, :] = [64338.27187, 36900.77477, 47182.78534, 52802.52599, 298986.6166, 731084.9614, 1055270.669, 1940635.146]
yData[0, 4, :] = [28424.46802, 39375.74472, 237406.821, 1027314.934, 2003857.809, 2042997.469, 1758470.575, 2012571.706]
yData[0, 5, :] = [2547665.889, 2079076.002, 2324980.48, 2178616.805, 1944435.717, 956261.5957, 1099876.149, 1250870.797]
...
def lnprior(args):
    ...
    if not (1E-9 <= k_on <= 1E-1 and 1E-5 <= k_off <= 1 and .01 <= kx_s_p <= 1.10 and 1E-5 <= b_e <= 0.1 and kx_wt_ml - 1.01*sigma_kx_wt <= kx_wt <= kx_wt_ml + 1.01*sigma_kx_wt and kx_rne_ml - 1.01*sigma_kx_rne <= kx_rne <= kx_rne_ml + 1.01*sigma_kx_rne and .1 - .010 <= a_m_rne <= a_m_rne_minus + sigma_a_m_rne + .010 and .1 - .010 <= a_m_wt <= a_m_wt_minus + sigma_a_m_wt + .010 and b_m <= b_ms <= 0.8):
        return -np.inf
...
def fixPos():
    for i in range(len(pos)):
        ...
        pos[i][4] = np.clip(pos[i][4], 0.1, a_m_wt_minus + sigma_a_m_wt)#a_m_wt
        pos[i][5] = np.clip(pos[i][5], 0.1, a_m_rne_minus + sigma_a_m_rne)#a_m_rne
