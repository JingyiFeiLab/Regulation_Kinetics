% Creates 2 output structures. 1) "cell_struct" : One entry for each cell containing its
% coordinates, center, boundaries, angle, and axis, and 2) "spot_struct"One entry
% for each spot containing the cell ID it belongs to, average diffusion coefficient, its 3D distance to
% center, and its distance to the membrane.

% This code will potentially take a while to run, depending on how many
% spots there are

% You should only have to change "pixelscaling", "msd_file", "dic_file",
...
%Set to diffusion tracking result .txt file
msd_file = '/Users/reyer/Documents/MATLAB/SOURCE_CODES/sample_images_matt/Matt_Microscope/JingyiTracking/msd.txt';
pixelscaling = .130; % .130 if um, 130 if nm
dataset=msd_file;                                          % Name of the datafile that you input....Lot of output files will have this "dataset" in their names
M=textread(dataset); 
dic_file = '/Users/reyer/Documents/MATLAB/SOURCE_CODES/sample_images_matt/Matt_Microscope/JingyiTracking/dic1.tif';
...
for i = 1:length(DiffCoef)
    ...
    for j = 1:max(mask(:))
        ...
        if mask_j(spot_x,spot_y) > 0
            ...
            spot_struct(i).Distance2Center(1) = abs(spot_struct(i).Transform_3D_Coordinate(1)/cell_struct(j).Cell_X_Axis);
            spot_struct(i).Distance2Center(2) = abs(spot_struct(i).Transform_3D_Coordinate(2)/cell_struct(j).Cell_Y_Axis);
            spot_struct(i).Distance2Center(3) = abs(spot_struct(i).Transform_3D_Coordinate(3)/cell_struct(j).Cell_X_Axis);
