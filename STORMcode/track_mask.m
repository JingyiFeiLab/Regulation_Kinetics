...
shape2D_thresh = 1.5;  % <--- Splitting threshold, you can change this

background_thresh = .05;
...
for i = 1:max(max(b))
    strcat(['Calculating Background Intensity ', num2str(i), ' ... '])
...
for i = 1:num
    strcat(['Calculating Ellipticity, Round 1 Cell ', num2str(i), ' ... '])
...
for i = 1:num
    strcat(['Making Ellipse, Round 1 Cell ', num2str(i), ' ... '])
    ...
    if ellipse_error(i) < shape2D_thresh
        ...
        objects = smallID(imfill(objects),10);
...
objects2 = bwlabel(smallID(bwlabel(I2,4),10),4);
num = max(objects2(:));
...
for i = 1:num
    strcat(['Calculating Background Intensity, Round 2 Cell ', num2str(i), ' ... '])
...
for i = 1:num
    strcat(['Calculating Ellipticity, Round 2 Cell ', num2str(i), ' ... '])
...
for i = 1:num
    strcat(['Making Ellipses, Round 2 Cell ', num2str(i), ' ... '])
    ...
    if ellipse_error(i) < shape2D_thresh
        ...
        objects2 = smallID(imfill(objects2),50);
    else
        objects2(objects2==i) = 0;
    end
end
