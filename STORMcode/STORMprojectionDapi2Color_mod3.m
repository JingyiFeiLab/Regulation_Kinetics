...
dataset='\\desktop-lkqeepp\G\Seongjin\AnalysisComputer2\2019_01_17_SP151_aTC_rRNA_Hfq_DAPI_fixed\mMaple3_3\output_all.txt';                                          % Name of the datafile that you input....Lot of output files will have this "dataset" in their names
%set "dic_file" to image containg the dic image

dic_file = '\\desktop-lkqeepp\G\Seongjin\AnalysisComputer2\2019_01_17_SP151_aTC_rRNA_Hfq_DAPI_fixed\DIC_3_crop.tif';
dapi_file = '\\desktop-lkqeepp\G\Seongjin\AnalysisComputer2\2019_01_17_SP151_aTC_rRNA_Hfq_DAPI_fixed\rRNA_sample_3';

stack_dapi = imFormat_wholeImage(dapi_file,1);
dapi_objects = riboMask(stack_dapi);
...
green_z=green_particles(:,z_col)*0;                          % Select the column containing Z coordinates in Pixel form
 
green_start = length(red_particles);
...
%imshow(mask);hold on;scatter(red_y,red_x,0.5,'filled','r');hold on;scatter(green_y,green_x,0.5,'g');
...
for i = 1:max(new_mask(:))
    ...
    for gi = 1:length(green_spot_coords)-1
        if sum(cell_row==green_spot_coords(gi,2) & cell_col==green_spot_coords(gi,3)) == 1
            ...
            spot_struct(green_spot_coords(gi,1)+green_start).Coordinate = [green_x(green_spot_coords(gi,1)),green_y(green_spot_coords(gi,1)),green_z(green_spot_coords(gi,1))];
            spot_struct(green_spot_coords(gi,1)+green_start).Cell = i;
    ...
    close all
    scatter(red_cell_spot_coords(:,2),red_cell_spot_coords(:,3),5,'filled','r');
    hold on
    scatter(green_cell_spot_coords(:,2),green_cell_spot_coords(:,3),5,'filled','g');
