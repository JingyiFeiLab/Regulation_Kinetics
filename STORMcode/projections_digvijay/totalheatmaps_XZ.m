function totalheatmaps_XZ(heatmapbinx,heatmapbiny,timelapse,heatmapanalysis_on_or_off)
close all;
% Example of Usage :
% totalheatmaps_XZ(30,30,10,'on');
...
% will be saved.
% 
% This process will be repeated for each XZ,XZ and XZ.
...
finalxprojected_StormsXZPlane=[];
finalyprojected_StormsXZPlane=[];
finalzprojected_StormsXZPlane=[];
...
y=data(:,2);
z=data(:,3);
finalxprojected_StormsXZPlane=[finalxprojected_StormsXZPlane ;x];    
finalyprojected_StormsXZPlane=[finalyprojected_StormsXZPlane ;y];  
finalzprojected_StormsXZPlane=[finalzprojected_StormsXZPlane ;z];  
end
close all;
...
% Z correction
    ...
    close all;
    plot(finalxprojected_StormsXZPlane,finalzprojected_StormsXZPlane,'ro');
    xlabel('X Axis');     
    ylabel('Z Axis');
    ...
    movegui(h,'northwest'); 
    figure
    [n,c] = hist3([finalxprojected_StormsXZPlane finalzprojected_StormsXZPlane],[heatmapbinx,heatmapbiny]);
    n=rot90(n,1);
    n = flipud(n);
    noriginal=flipud(n);
    ...
    finalzprojected_StormsXZPlane=finalzprojected_StormsXZPlane+ZCorrection;
    close all;
    plot(finalxprojected_StormsXZPlane,finalzprojected_StormsXZPlane,'ro');
    xlabel('X Axis');     
    ylabel('Z Axis');
...
projectedpoints_write=[finalxprojected_StormsXZPlane finalyprojected_StormsXZPlane finalzprojected_StormsXZPlane];
projectedpoints_write_StormsXZPlane=sprintf('%s_Combined_XZPlane.dat',datasetname);
dlmwrite(projectedpoints_write_StormsXZPlane,projectedpoints_write,' ');
close all;

imagewindow = figure('visible','on');
plot(finalxprojected_StormsXZPlane,finalzprojected_StormsXZPlane,'ro');
saveas(imagewindow,'finalprojectpoints_on_StormsXZplane.jpg');
imagewindow = figure('visible','on');
[n,c] = hist3([finalxprojected_StormsXZPlane finalzprojected_StormsXZPlane],[heatmapbinx,heatmapbiny]);
n=rot90(n,1);
n = flipud(n);
...
close all;
heatmaptext(noriginal); title('Horizontal axis is X and Vertical axis is Z')
colormapeditor
...
projectpoints_on_StormsXZplaneHeatMap=strcat(datasetname,'_TotalFinal_projectpoinmsXZplane_HeatMap.jpg');
saveas(imagewindow,projectpoints_on_StormsXZplaneHeatMap);
projectpoints_on_StormsXZplaneHeatMap=strcat(datasetname,'_TotalFinal_projectpoinmsXZplane_HeatMap.fig');
saveas(imagewindow,projectpoints_on_StormsXZplaneHeatMap);
end