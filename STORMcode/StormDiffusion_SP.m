...
pixelscaling = .0867; % Sometimes Seongjin's data is in nm, sometimes in um. If nm, set to 130. If um, set to .130
% set "dataset" to STORM output .txt file
msd_file = '/Users/reyer/Documents/MATLAB/SOURCE_CODES/sample_images_matt/Matt_Microscope/reurgent/onestep_MSD_list_min_3.txt';
dataset=msd_file;

%set "dic_file" to image containg the dic image
dic_file = '/Users/reyer/Documents/MATLAB/SOURCE_CODES/sample_images_matt/Matt_Microscope/reurgent/dic1.tif';
M=textread(dataset); 
...
x=x./(pixelscaling/binning);                     % Scale it up...Pixel Scaling is to convert coordinates in nm,Angstrom to Pixel Coordinates
y=y./(pixelscaling/binning);                     % Scale it up...Pixel Scaling is to convert coordinates in nm,Angstrom to Pixel Coordinates
z = z./pixelscaling;                  
...
for i = 1:max(mask(:))
    ...
    cell_struct(i).Cell_Y_Axis = max(cell_struct(i).Transformed_Boundaries{1,1}(:,1))-min(cell_struct(i).Transformed_Boundaries{1,1}(:,1));
    cell_struct(i).Cell_X_Axis = max(cell_struct(i).Transformed_Boundaries{1,1}(:,2))-min(cell_struct(i).Transformed_Boundaries{1,1}(:,2));
    ...
    cell_spot_coords = zeros(length(cell_spots),9);
    ...
    for gs = cell_spots
        ...
        cell_spot_coords(qr,:) = [gs spot_struct(gs).Transform_3D_Coordinate(1) spot_struct(gs).Transform_3D_Coordinate(2) spot_struct(gs).Transform_3D_Coordinate(3) spot_struct(gs).Region spot_struct(gs).DiffusionCoefficient spot_struct(gs).Coordinate(1) spot_struct(gs).Coordinate(2) spot_struct(gs).Coordinate(3)];
        qr = qr+1;
    end
    ...
    cell_spot_coords(cell_spot_coords(:,1)==0,:) = [];
