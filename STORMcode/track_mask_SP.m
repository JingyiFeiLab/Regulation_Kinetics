...
background_thresh = .1; % lower is generous (default=.1~.2) .1
...
for i = 1:num
    ...
    if isempty(ellipse1) == 1 || isempty(test1) == 1
        ...
        continue
    elseif (isempty(ellipse1) == 1 && cellArea(objects,i) > 200) || (isempty(test1) == 1 && cellArea(objects,i) > 200)
        ellipse_error(i) = 11;
        continue
    else
        i
...
for i = 1:num
    ...
    if ellipse_error(i) < shape2D_thresh
        ...
        objects2 = smallID(imfill(objects2),10);
    else
        objects2(objects2==i) = 0;
    end
end
