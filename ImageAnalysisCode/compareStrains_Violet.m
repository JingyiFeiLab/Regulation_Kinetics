...
strain = {};
time = {};
Dates = {'June_19_2018','May_26_2018'};
newFile = 'WT_transcription_curves';
graph_title = 'manX';
...
if GFP == 1 && sigma_check == 0
    ...
    title(strcat([graph_title,' Protein']),'FontSize',32)
    xlabel('Time after Induction (min)','FontSize',24)
    xt = get(gca, 'XTick');
    set(gca, 'FontSize', 18)
    ax = gca;
    ax.YLim = [4.40E3 1.03E6];
...
if SGRS == 1 && sigma_check == 0
    ...
    title(strcat([graph_title,' sRNA']),'FontSize',32)
    xlabel('Time after Induction (min)','FontSize',24)
    xt = get(gca, 'XTick');
    set(gca, 'FontSize', 18)
    ax = gca;
    ax.YLim = [4.33E5 2.83E6];
...
if PTSG == 1 && sigma_check == 0
    ...
    title(strcat([graph_title,' mRNA']),'FontSize',32)
    xlabel('Time after Induction (min)','FontSize',24)
    xt = get(gca, 'XTick');
    set(gca, 'FontSize', 18)
    ax = gca;
    ax.YLim = [1.86E4 2.89E5];
