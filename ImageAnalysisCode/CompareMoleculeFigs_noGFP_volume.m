...
time = {};
Dates = {'April_12_2018'};
newFile = 'Plus_SgrS_mRNA_pre_Induced_Slide_Background';
...
volumeDir = strcat([compareDir,'/Volume']);
        if exist(volumeDir,'dir')~=7
            mkdir(compareDir,'Volume')
        end
...
for i = 1:length(strains)
    ...
    dataFile = strcat([dataDir,'VolNormSummaryData.mat']);
    load(dataFile)
    time{i} = int_table{:,1};
...
if GFP == 0 && PTSG == 1 && SGRS == 1 && sigma_check == 0
    ...
    for i = 1:length(strains)
        ...
        [ax,hlines(1),hlines(2)] = plotyy(time{i},300*mean_ptsg{i},time{i},300*mean_sgrs{i});
        cfig = get(gcf,'color');
        pos = [0.1  0.1  0.7  0.8];
        offset = pos(3)/5.5;
        ...
        set(gcf,'PaperPositionMode','auto')
        print(file1,'-dpng','-r0')
        file1_fig = strcat([volumeDir,'/',strain{i},'_molecule.fig']);
    ...
    for i = 1:length(strains)
        ...
        [ax,hlines(1),hlines(2)] = plotyy(time{i},315*mean_green{i}/green_back(i),time{i},315*mean_red{i});
        cfig = get(gcf,'color');
        pos = [0.1  0.1  0.7  0.8];
        offset = pos(3)/5.5;
        hlines(1).LineWidth = 6;
        ...
        lgd2.FontSize = 14;
        file1 = strcat([volumeDir,'/',strain{i},'_intensity']);
        set(gcf,'PaperPositionMode','auto')
        print(file1,'-painters','-depsc','-r0')
        set(gcf,'PaperPositionMode','auto')
        print(file1,'-dpng','-r0')
