...
folderTitle = '/Users/reyer/Data/SingleCellEpi/MR162/am2/';
parentDir = '/Users/reyer/Data/SingleCellEpi/';
strains = [162];
time = {};
DatesV = {'July_13_2020_2','July_13_2020_2'};
graph_title = ' rne701 ptsG, - sRNA';
...
funcToFitM = @(x0,time) nosRNA_fit(x0,time,0,0,0,0,0,b_m,0,0,b_p,molecules0M);
funcToFitP = @(x0,time) nosRNA_fit(x0,time,0,0,0,0,0,b_m,0,0,b_p,molecules0M);
funcToFit = @(x0,time) nosRNA_fit(x0,time,0,0,0,0,0,b_m,0,0,b_p,molecules0M);
...
funcWeightedLeastSquares = @(x) sum([sum(((1/max(compare_green(:)))*(subindex(funcToFitM(x, time(mRNA_indices)), ':', 2) - ydata(mRNA_indices,2)).^2)),sum(((1/max(compare_blue(:)))*(subindex(funcToFitP(x, time_shift(protein_indices)), 1:length(protein_indices), 4) - ydata(protein_indices,3)).^2))]);
...
Cfit = nosRNA_fit(xMin, tv,0,0,0,0,0,b_m,0,0,b_p,molecules0M);
Cfit2 = nosRNA_fit(xMin, tv2,0,0,0,0,0,b_m,0,0,b_p,molecules0P);
...
hold on
for ig = 1:length(DatesV)
    scatter(time,compare_red(ig,:),'or','LineWidth',3)
    hold on
end
title(strcat([graph_title,'sRNA Fit',DatesV]),'FontSize',32)
xlabel('Time after Induction (min)','FontSize',24)
% set(gca,'YLim',[0 2E6])
% set(gca,'XLim',[0 12])
set(gca,'FontSize',18)
...
lgd.Location = 'northeast';
lgd.FontSize = 14;
file1 = strcat([compareDir,'/',strain,'sRNA_fit']);
set(gcf,'PaperPositionMode','auto')
print(file1,'-painters','-depsc','-r0')
set(gcf,'PaperPositionMode','auto')
print(file1,'-dpng','-r0')
