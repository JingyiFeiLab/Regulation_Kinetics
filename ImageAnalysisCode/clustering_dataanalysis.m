function clustering_dataanalysis(datafile,ColorCode,Start_Npts,End_Npts,Increment_Npts,Start_Eps,End_Eps,Increment_Eps,Window_Status,imagestatus,moviestatus)
...
M=textread(dataset);                                       % Reads in the dataset
...
for t=1:numel(M)/4
if M(t,4)==ColorCode
data=[data ;[M(t,1) M(t,2) M(t,3)]];                       % So here's how I have saved the points belonging to a colorcode from the original dataset
end
end
if ColorCode==0
    data=M;
...
averagefile=sprintf('./%s_ColorCode%d_AverageSheet.csv',dataset,ColorCode);
for Npts=Start_Npts:Increment_Npts:End_Npts                    % The Npts parameter of the DBSCAN
    for Eps=Start_Eps:Increment_Eps:End_Eps                    % The Eps  parameter of the DBSCAN
        ...
        cluster                                       % A script which does all the required analysis...so any additional analysis or modifications in analysis required has to made in this script
