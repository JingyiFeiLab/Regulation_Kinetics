...
while continuetranslation==1
    prompt={'X_Translation:','Y_Translation:','Continue the translation(1 == Yes && 0== NO) ?'};   
    % A box will take in the values for the X/Ytranslation
    ...
    %x=x-Xtranslation;  % Translated==>This line caused troubles for
    %colocalization analysis, so being removed (Aug20,2016, Seongjin)
    %y=y-Ytranslation;
    %%%%%%%%%%%%% instered by Seongjin to translace DIC image instead
    image_trans=image;
    for j=1+abs(Xtranslation):X-abs(Xtranslation)
        for i=1+abs(Ytranslation):Y-abs(Ytranslation)
            image_trans(i,j)=image(i+Ytranslation,j-Xtranslation);
        end
    end
    image=image_trans;
    imwrite(image_trans,'DIC_trans.jpg');
    referenceimage='DIC_trans.jpg';
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
CellAnalysis=[CellAnalysis; [count length(find(ColumnCell==1)) length(find(ColumnCell==2)) length(find(ColumnCell==3)) length(find(ColumnCell==4)) ...
    ...
    Total_Numberof_Spots_1 Total_Numberof_Spots_2 Total_Numberof_Spots_3 Total_Numberof_Spots_4 Total_Numberof_Spots_5 Total_Numberof_Spots_6...
    Total_Numberof_Spots_7 Total_Numberof_Spots_8 Total_Numberof_Spots_9 polyarea(Xpoly,Ypoly) X Y]]; %check the color number
