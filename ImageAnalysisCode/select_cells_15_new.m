function select_cells_9_new(coordinatefile,Columnx,Columny,Columnz,ColumnR,Column_No_Spots,ColorLineColumn,referenceimage,pixelscaling)
% brifly corrected by Seongjin (the first row, and line 68)
...
%******EXAMPLE OF USAGE
% select_cells_9_new('DJ480_0min_total_centers.dat',1,2,3,4,5,6,'film20.jpg',100);
% Where 
...
points=[x y z AverageR No_Spots];
...
while count < cellcount                          % Allows you to select upto "cellcount" cells in the image through polygonal selections of the area
    ...
    Total_Numberof_Spots_6=0;
    Total_Numberof_Spots_7=0;
    Total_Numberof_Spots_8=0;
    Total_Numberof_Spots_9=0;
    Total_Numberof_Spots_10=0;
    Total_Numberof_Spots_11=0;
    Total_Numberof_Spots_12=0;
    Total_Numberof_Spots_13=0;
    Total_Numberof_Spots_14=0;
    Total_Numberof_Spots_15=0;
    ...
    for i=1:numel(selected_indices)
      if selected_indices(i)==1
        temporarypoints=[temporarypoints; [newxy(i,1) newxy(i,2) newxy(i,3) newxy(i,4) newxy(i,5) Color(i)]];  % Making a list of the points within the selected area
        ...
        if Color(i)==6
        Total_Numberof_Spots_6=Total_Numberof_Spots_6+newxy(i,5);
        end
        
        if Color(i)==7
        Total_Numberof_Spots_7=Total_Numberof_Spots_7+newxy(i,5);
        end
        
        if Color(i)==8
        Total_Numberof_Spots_8=Total_Numberof_Spots_8+newxy(i,5);
        end
        
        if Color(i)==9
        Total_Numberof_Spots_9=Total_Numberof_Spots_9+newxy(i,5);
        end
        
        if Color(i)==10
        Total_Numberof_Spots_10=Total_Numberof_Spots_10+newxy(i,5);
        end
        
        if Color(i)==11
        Total_Numberof_Spots_11=Total_Numberof_Spots_11+newxy(i,5);
        end
        
        if Color(i)==12
        Total_Numberof_Spots_12=Total_Numberof_Spots_12+newxy(i,5);
        end
        
        if Color(i)==13
        Total_Numberof_Spots_13=Total_Numberof_Spots_13+newxy(i,5);
        end
        
        if Color(i)==14
        Total_Numberof_Spots_14=Total_Numberof_Spots_14+newxy(i,5);
        end
        
        if Color(i)==15
        Total_Numberof_Spots_15=Total_Numberof_Spots_15+newxy(i,5);
        end
...
CellAnalysis=[CellAnalysis; [count length(find(ColumnCell==1)) length(find(ColumnCell==2)) length(find(ColumnCell==3)) length(find(ColumnCell==4)) ...
    length(find(ColumnCell==5)) length(find(ColumnCell==6)) length(find(ColumnCell==7)) length(find(ColumnCell==8)) length(find(ColumnCell==9))...
    length(find(ColumnCell==10)) length(find(ColumnCell==11)) length(find(ColumnCell==12))...
    length(find(ColumnCell==13)) length(find(ColumnCell==14)) length(find(ColumnCell==15))...    
    Total_Numberof_Spots_1 Total_Numberof_Spots_2 Total_Numberof_Spots_3... 
    Total_Numberof_Spots_4 Total_Numberof_Spots_5 Total_Numberof_Spots_6...
    Total_Numberof_Spots_7 Total_Numberof_Spots_8 Total_Numberof_Spots_9... 
    Total_Numberof_Spots_10 Total_Numberof_Spots_11 Total_Numberof_Spots_12...  
    Total_Numberof_Spots_13 Total_Numberof_Spots_14 Total_Numberof_Spots_15...
    polyarea(Xpoly,Ypoly) X Y]]; %check the color number

filename=sprintf('cell_%d.dat',count);
